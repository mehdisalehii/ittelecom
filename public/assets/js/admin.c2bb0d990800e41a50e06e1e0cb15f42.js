var choosePrimaryImageFunction;
...
;(function ($, window, document, undefined) {
    ...
    var defaults = {
        ...
        emptyClass: 'dd-empty',
        expandBtnHTML: '<button data-action="expand" type="button" title="باز">Expand</button>',
        collapseBtnHTML: '<button data-action="collapse" type="button" title="بسته">Collapse</button>',
        group: 0,
        maxDepth: 10,
        threshold: 20
    };
    ...
    Plugin.prototype = {
        ...
        init: function () {
            ...
            list.el.data('menuable-group', this.options.group);
        ...
        dragMove: function (e) {
            ...
            var pointElRoot = this.pointEl.closest('.' + opt.rootClass),
                isNewRoot = this.dragRootEl.data('menuable-id') !== pointElRoot.data('menuable-id');
            if (!mouse.dirAx || isNewRoot || isEmpty) {
                if (isNewRoot && opt.group !== pointElRoot.data('menuable-group')) {
                    return;
                }
    ...
    $.fn.menuable = function (params) {
        var lists = this,
            retval = this;

        lists.each(function () {
            var plugin = $(this).data("menuable");

            if (!plugin) {
                $(this).data("menuable", new Plugin(this, params));
                $(this).data("menuable-id", new Date().getTime());
            } else {
...
$(document).ready(function () {
    ...
    var updateOutput = function (e) {
        ...
        if (output) {
            if (window.JSON) {
                output.val(window.JSON.stringify(list.menuable('serialize')));
            } else {
    ...
    updateOutput($('#menuable').data('output', $('#menuable-output')));

    $('.dd').menuable('collapseAll');
    $('.collapse-all').hide();

    $(document).on("click", "#menuable-menu", function (e) {
        var target = $(e.target),
            action = target.data('action');
        if (action === 'expand-all') {
            $('.dd').menuable('expandAll');
            $('.expand-all').hide();
            $('.collapse-all').show();
...
$(document).ready(function () {
    ...
    $("#submit").click(function () {
        ...
        var val_data = {
            ...
            content: $("#content").val(),
            content_bottom: $("#content_bottom").val(),
            type: $("#type").val(),
            sku: $("#sku").val(),
            id: $("#id").val()
        };

        const CSRF_TOKEN = $('.data_href').attr('csrf-token');

        $.ajax({
            type: "POST",
            url: 'admin/category/save_menu' + '?CSRF_TOKEN=' + CSRF_TOKEN + '&_=' + Math.random(),
            data: {"_token": CSRF_TOKEN, val_data},
            type: 'POST',
            cache: false,
            success: function (data) {
                if (val_data.id) {
                    $('#label_show' + val_data.id).html(val_data.label);
                    $('#slug_show' + val_data.id).html(val_data.slug);
                    $('#thumb_show' + val_data.id).html(val_data.thumb);
                    $('#class_name_show' + val_data.id).html(val_data.class_name);
                    $('#content_show' + val_data.id).html(val_data.content.replace(/<[^>]*>?/gm, ''));
                    $('#content_bottom_show' + val_data.id).html(val_data.content_bottom.replace(/<[^>]*>?/gm, ''));
                    $('#sku_show' + val_data.id).html(val_data.sku);
                    $('#type_show' + val_data.id).html(val_data.type);

                    $('.edit-button[id="' + val_data.id + '"]').attr('label', val_data.label)
                    $('.edit-button[id="' + val_data.id + '"]').attr('slug', val_data.slug)
                    $('.edit-button[id="' + val_data.id + '"]').attr('thumb', val_data.thumb)
                    $('.edit-button[id="' + val_data.id + '"]').attr('class_name', val_data.class_name)
                    $('.edit-button[id="' + val_data.id + '"]').attr('content', val_data.content.replace(/<[^>]*>?/gm, ''))
                    $('.edit-button[id="' + val_data.id + '"]').attr('content_bottom', val_data.content_bottom.replace(/<[^>]*>?/gm, ''))
                    $('.edit-button[id="' + val_data.id + '"]').attr('sku', val_data.sku)
                    $('.edit-button[id="' + val_data.id + '"]').attr('type', val_data.type)

                } else {
                    window.location = "admin/category/product";
                }
    ...
    $('.dd').on('change', function () {
        $("#load").show();
        var data = $("#menuable-output").val();
        const CSRF_TOKEN = $('.data_href').attr('csrf-token');
        $.ajax({
            type: "POST",
            url: 'admin/category/save' + '?CSRF_TOKEN=' + CSRF_TOKEN + '&_=' + Math.random(),
    ...
    $(document).on("click", ".del-button", function () {
        ...
        AlertConfirm('حذف', 'آیا میخواهید شماره ' + id + ' را حذف شود؟', 'بله!')
        ...
        $(document).ready(function () {
            $('.btn-yes-done').on('click', function () {
                ...
                $.ajax({
                    type: "POST",
                    url: 'admin/category/delete' + '?CSRF_TOKEN=' + CSRF_TOKEN + '&_=' + Math.random(),
    ...
    function TinyEditor() {
        ...
        tinymce.init({
            ...
            toolbar1: 'bold italic underline formatselect outdent indent numlist bullist rtl ltr alignright aligncenter alignleft alignjustify link image table code blockquote removeformat',
            menubar: false,
            directionality: $('.direction').attr('data-dir'),
            contextmenu: false,
            elementpath: false,
            resize: false,
            language: 'fa_IR',
            ...
            images_upload_url: 'admin/upload/fetch',

            file_picker_callback: function (callback, value, meta) {
                var x = window.innerWidth || document.documentElement.clientWidth || document.getElementsByTagName('body')[0].clientWidth;
                var y = window.innerHeight || document.documentElement.clientHeight || document.getElementsByTagName('body')[0].clientHeight;

                var cmsURL = 'files?editor=' + meta.fieldname;
            ...
            plugins: [
                'link image imagetools table spellchecker lists directionality code textcolor '
            ],

            autoresize_bottom_margin: 10,
            contextmenu: 'bold image removeformat fontselect table formatselect h1 italic lineheight undo redo styleselect alignleft aligncenter alignright alignjustify outdent indent',
            ...
            setup: function (editor) {
                ...
                editor.ui.registry.addContextToolbar('textselection', {
                    ...
                    items: 'removeformat | bold underline | indent | numlist bullist | table | rtl ltr | link | alignright aligncenter alignleft alignjustify',
                    position: 'selection',
                    scope: 'node'
                });
            ...
            plugins: [
                'link image imagetools table spellchecker lists directionality autoresize code'
            ],
            autoresize_bottom_margin: 10,
            content_css: "/assets/css/tiny.css",
            content_style: "body { font-family:font,Helvetica,Arial,sans-serif; font-size:18px }.mce-content-body{padding:15px}.mce-content-body p{line-height:2.5rem;text-align:justify;font-size:1.25rem;padding:0}h1,h2,h3,h4,h5,h6{margin:0;padding:0;font-size:1.5rem;font-family:sbold,tahoma;line-height:2rem;font-weight:700;}p,blockquote{margin:0;padding:0}ul{margin:0}ul li{margin:0 30px}",
        });
    ...
    $(document).on("click", ".edit-button", function () {
        ...
        var content_bottom = $(this).attr('content_bottom');
        ...
        $("#content").val(content);
        $("#content_bottom").val(content_bottom);
        $("#sku").val(sku);
        $("#type").val(type);
        $('.option label.txt').addClass('active')
    ...
    $(document).on("click", "#reset", function () {
        ...
        $('#type').val('product');
        $('#id').val('');
        $('.option label.txt').removeClass('active')
        $('.option label.txt[id="thumb"]').addClass('active')
    });

    $(document).on("click", ".dd-handle-disable", function () {
        alert('امکان جابجایی توسط ادمین انجام می‌شود.')
    });
...
!function (t, e) {
    ...
    var B = {
        toLineHeight: function (t, e) {
            ...
            var e = z.global, n = N(t.fontSize, e.defaultFontSize), i = {family: N(t.fontFamily, e.defaultFontFamily), lineHeight: M.options.toLineHeight(N(t.lineHeight, e.defaultLineHeight), n), size: n, style: N(t.fontStyle, e.defaultFontStyle), weight: null, string: ""};
    ...
    var X = G, K = X.extend({chart: null, currentStep: 0, numSteps: 60, easing: "", render: null, onAnimationProgress: null, onAnimationComplete: null}), Z = K;
    ...
    V.extend(et.prototype, {
        datasetElementType: null, dataElementType: null, _datasetElementOptions: ["backgroundColor", "borderCapStyle", "borderColor", "borderDash", "borderDashOffset", "borderJoinStyle", "borderWidth"], _dataElementOptions: ["backgroundColor", "borderColor", "borderWidth", "pointStyle"], initialize: function (t, e) {
            var n = this;
            ...
            n._data !== a && (n._data && tt(n._data, n), a && Object.isExtensible(a) && (e = n, (t = a)._chartbrjs ? t._chartbrjs.listeners.push(e) : (Object.defineProperty(t, "_chartbrjs", {configurable: !0, enumerable: !1, value: {listeners: [e]}}), Q.forEach((function (e) {
                ...
                Object.defineProperty(t, e, {
                    configurable: !0, enumerable: !1, value: function () {
                        ...
                        return V.each(t._chartbrjs.listeners, (function (t) {
        ...
        }, update: V.noop, transition: function (t) {
            ...
            var n, i, a, r, o = this, s = o.chart, l = o._config, u = t.custom || {}, d = s.options.elements[o.datasetElementType.prototype._type] || {}, h = o._datasetElementOptions, c = {}, f = {chart: s, dataset: o.getDataset(), datasetIndex: o.index, hover: e};
    ...
    var ot = X.extend({
        _type: "arc", inLabelRange: function (t) {
            ...
            var t, e = this._chart.ctx, n = this._view, i = "inner" === n.borderAlign ? .33 : 0, a = {x: n.x, y: n.y, innerRadius: n.innerRadius, outerRadius: Math.max(n.outerRadius - i, 0), pixelMargin: i, startAngle: n.startAngle, endAngle: n.endAngle, fullCircles: Math.floor(n.circumference / it)};
    ...
    function mt(t) {
        var e, n, i, a, r;
        return pt(t) ? (r = t.width / 2, e = t.x - r, n = t.x + r, i = Math.min(t.y, t.base), a = Math.max(t.y, t.base)) : (r = t.height / 2, e = Math.min(t.x, t.base), n = Math.max(t.x, t.base), i = t.y - r, a = t.y + r), {left: e, top: i, right: n, bottom: a}
    }
    ...
    var Dt = nt.extend({
        dataElementType: _t.Rectangle, _dataElementOptions: ["backgroundColor", "borderColor", "borderSkipped", "borderWidth", "barPercentage", "barThickness", "categoryPercentage", "maxBarThickness", "minBarLength"], initialize: function () {
            var t, e, n = this;
    ...
    var Ft = nt.extend({
        dataElementType: _t.Point, _dataElementOptions: ["backgroundColor", "borderColor", "borderWidth", "hoverBackgroundColor", "hoverBorderColor", "hoverBorderWidth", "hoverRadius", "hitRadius", "pointStyle", "rotation"], update: function (t) {
            ...
            var n = this, i = n.chart, a = n.getDataset(), r = t.custom || {}, o = a.data[e] || {}, s = nt.prototype._resolveDataElementOptions.apply(n, arguments), l = {chart: i, dataIndex: e, dataset: a, datasetIndex: n.index};
    ...
    var Nt = nt.extend({
        dataElementType: _t.Arc, linkScales: V.noop, _dataElementOptions: ["backgroundColor", "borderColor", "borderWidth", "borderAlign", "hoverBackgroundColor", "hoverBorderColor", "hoverBorderWidth"], getRingIndex: function (t) {
    ...
    var Ut = nt.extend({
        datasetElementType: _t.Line, dataElementType: _t.Point, _datasetElementOptions: ["backgroundColor", "borderCapStyle", "borderColor", "borderDash", "borderDashOffset", "borderJoinStyle", "borderWidth", "cubicInterpolationMode", "fill"], _dataElementOptions: {backgroundColor: "pointBackgroundColor", borderColor: "pointBorderColor", borderWidth: "pointBorderWidth", hitRadius: "pointHitRadius", hoverBackgroundColor: "pointHoverBackgroundColor", hoverBorderColor: "pointHoverBorderColor", hoverBorderWidth: "pointHoverBorderWidth", hoverRadius: "pointHoverRadius", pointStyle: "pointStyle", radius: "pointRadius", rotation: "pointRotation"}, update: function (t) {
    ...
    var Gt = nt.extend({
        dataElementType: _t.Arc, linkScales: V.noop, _dataElementOptions: ["backgroundColor", "borderColor", "borderWidth", "borderAlign", "hoverBackgroundColor", "hoverBorderColor", "hoverBorderWidth"], _getIndexScaleId: function () {
    ...
    z._set("scatter", {
        hover: {mode: "single"}, scales: {xAxes: [{id: "x-axis-1", type: "linear", position: "bottom"}], yAxes: [{id: "y-axis-1", type: "linear", position: "left"}]}, tooltips: {
            callbacks: {
                title: function () {
                    return ""
                }, label: function (t) {
    ...
    var fe, ge = {
        defaults: {}, addBox: function (t, e) {
            ...
            if (t) {
                ...
                }(t.boxes), l = s.vertical, u = s.horizontal, d = Object.freeze({outerWidth: e, outerHeight: n, padding: a, availableWidth: r, vBoxMaxWidth: r / 2 / l.length, hBoxMaxHeight: o / 2}), h = re({maxPadding: re({}, a), w: r, h: o, x: a.left, y: a.top}, a);
                !function (t, e) {
                    var n, i, a;
    ...
    }, pe = (fe = Object.freeze({__proto__: null, default: "@keyframes chartbrjs-render-animation{from{opacity:.99}to{opacity:1}}.chartbrjs-render-monitor{animation:chartbrjs-render-animation 1ms}.chartbrjs-size-monitor,.chartbrjs-size-monitor-expand,.chartbrjs-size-monitor-shrink{position:absolute;direction:ltr;left:0;top:0;right:0;bottom:0;overflow:hidden;pointer-events:none;visibility:hidden;z-index:-1}.chartbrjs-size-monitor-expand>div{position:absolute;width:1000000px;height:1000000px;left:0;top:0}.chartbrjs-size-monitor-shrink>div{position:absolute;width:200%;height:200%;left:0;top:0}"})) && fe.default || fe, me = "$chartbrjs", ve = "chartbrjs-size-monitor", be = "chartbrjs-render-monitor", xe = "chartbrjs-render-animation", ye = ["animationstart", "webkitAnimationStart"], _e = {touchstart: "mousedown", touchmove: "mousemove", touchend: "mouseup", pointerenter: "mouseenter", pointerdown: "mousedown", pointermove: "mousemove", pointerup: "mouseup", pointerleave: "mouseout", pointerout: "mouseout"};
    ...
    var Te = {
        disableCSSInjection: !1, _enabled: "undefined" != typeof window && "undefined" != typeof document, _ensureLoaded: function (t) {
            if (!this.disableCSSInjection) {
                var e = t.getRootNode ? t.getRootNode() : document;
                !function (t, e) {
                    var n = t[me] || (t[me] = {});
                    if (!n.containsStyles) {
                        n.containsStyles = !0, e = "/* Chartbr.js */\n" + e;
    ...
    var je = X.extend({
        initialize: function () {
            ...
            var e, n, i, a, r, o, s, l, u, d, h = this, c = h._options, f = h._model, g = h._model = We(c), p = h._active, m = h._data, v = {xAlign: f.xAlign, yAlign: f.yAlign}, b = {x: f.x, y: f.y}, x = {width: f.width, height: f.height}, y = {x: f.caretX, y: f.caretY};
            if (p.length) {
                g.opacity = 1;
                var _ = [], k = [];
    ...
    V.extend(nn.prototype, {
        formats: en, parse: en, format: en, add: en, diff: en, startOf: en, endOf: en, _create: function (t) {
            return t
        }
    ...
    z._set("scale", {display: !0, position: "left", offset: !1, gridLines: {display: !0, color: "rgba(0,0,0,0.1)", lineWidth: 1, drawBorder: !0, drawOnChartbrArea: !0, drawTicks: !0, tickMarkLength: 10, zeroLineWidth: 1, zeroLineColor: "rgba(0,0,0,0.25)", zeroLineBorderDash: [], zeroLineBorderDashOffset: 0, offsetGridLines: !1, borderDash: [], borderDashOffset: 0}, scaleLabel: {display: !1, labelString: "", padding: {top: 4, bottom: 4}}, ticks: {beginAtZero: !1, minRotation: 0, maxRotation: 50, mirror: !1, padding: 0, reverse: !1, display: !0, autoSkip: !0, autoSkipPadding: 0, labelOffset: 0, callback: rn.formatters.values, minor: {}, major: {}}});
    var bn = X.extend({
        zeroLineIndex: 0, getPadding: function () {
            ...
            var t = this, e = t.minSize = {width: 0, height: 0}, n = t.chart, i = t.options, a = i.ticks, r = i.scaleLabel, o = i.gridLines, s = t._isVisible(), l = "bottom" === i.position, u = t.isHorizontal();
    ...
    var Sn = xn.extend({
        getRightValue: function (t) {
            ...
            var t = this, e = t.options.ticks, n = t.getTickLimit(), i = {maxTicks: n = Math.max(2, n), min: e.min, max: e.max, precision: e.precision, stepSize: V.valueOrDefault(e.fixedStepSize, e.stepSize)}, a = t.ticks = function (t, e) {
    ...
    var Rn = xn.extend({
        determineDataLimits: function () {
            ...
            var t = this, e = t.options.ticks, n = !t.isHorizontal(), i = {min: On(e.min), max: On(e.max)}, a = t.ticks = function (t, e) {
    ...
    var Nn = V.valueOrDefault, Bn = V.valueAtIndexOrDefault, En = V.options.resolve, Wn = {
        display: !0, animate: !0, position: "chartArea", angleLines: {display: !0, color: "rgba(0,0,0,0.1)", lineWidth: 1, borderDash: [], borderDashOffset: 0}, gridLines: {circular: !1}, ticks: {showLabelBackdrop: !0, backdropColor: "rgba(255,255,255,0.75)", backdropPaddingY: 2, backdropPaddingX: 2, callback: rn.formatters.linear}, pointLabels: {
    ...
    var Gn = Sn.extend({
        setDimensions: function () {
            ...
            t.display && t.pointLabels.display ? function (t) {
                var e, n, i, a = V.options._parseFont(t.options.pointLabels), r = {l: 0, r: t.width, t: 0, b: t.height - t.paddingTop}, o = {};
    ...
    Gn._defaults = Xn;
    var Kn = V._deprecated, Zn = V.options.resolve, $n = V.valueOrDefault, Jn = Number.MIN_SAFE_INTEGER || -9007199254740991, Qn = Number.MAX_SAFE_INTEGER || 9007199254740991, ti = {millisecond: {common: !0, size: 1, steps: 1e3}, second: {common: !0, size: 1e3, steps: 60}, minute: {common: !0, size: 6e4, steps: 60}, hour: {common: !0, size: 36e5, steps: 24}, day: {common: !0, size: 864e5, steps: 30}, week: {common: !1, size: 6048e5, steps: 4}, month: {common: !0, size: 2628e6, steps: 12}, quarter: {common: !1, size: 7884e6, steps: 4}, year: {common: !0, size: 3154e7}}, ei = Object.keys(ti);
    ...
    di._defaults = hi;
    var ci = {category: _n, linear: Dn, logarithmic: Rn, radialLinear: Gn, time: di}, fi = {datetime: "MMM D, YYYY, h:mm:ss a", millisecond: "h:mm:ss.SSS a", second: "h:mm:ss a", minute: "h:mm a", hour: "hA", day: "MMM D", week: "ll", month: "MMM YYYY", quarter: "[Q]Q - YYYY", year: "YYYY"};
    ...
    z._set("global", {
        legend: {
            display: !0, position: "top", align: "center", fullWidth: !0, reverse: !1, weight: 1e3, onClick: function (t, e) {
    ...
    var Ii = X.extend({
        ...
        }, beforeUpdate: Ti, update: function (t, e, n) {
            var i = this;
            return i.beforeUpdate(), i.maxWidth = t, i.maxHeight = e, i.margins = n, i.beforeSetDimensions(), i.setDimensions(), i.afterSetDimensions(), i.beforeBuildLabels(), i.buildLabels(), i.afterBuildLabels(), i.beforeFit(), i.fit(), i.afterFit(), i.afterUpdate(), i.minSize
        }, afterUpdate: Ti, beforeSetDimensions: Ti, setDimensions: function () {
            var t = this;
            t.isHorizontal() ? (t.width = t.maxWidth, t.left = 0, t.right = t.width) : (t.height = t.maxHeight, t.top = 0, t.bottom = t.height), t.paddingLeft = 0, t.paddingTop = 0, t.paddingRight = 0, t.paddingBottom = 0, t.minSize = {width: 0, height: 0}
        }, afterSetDimensions: Ti, beforeBuildLabels: Ti, buildLabels: Ti, afterBuildLabels: Ti, beforeFit: Ti, fit: function () {
        ...
        }, afterFit: Ti, isHorizontal: function () {
            var t = this.options.position;
            return "top" === t || "bottom" === t
        }, draw: function () {
...
function PopUp() {
    $(document).on('click', '.btn-popup', function (event) {
        ...
        $('body main .pageblack').remove()
        $('body main .popup-show').remove()
        $('body main').append('<div class="pageblack back-intro"></div>').fadeIn(325);
        $('body').addClass('lock-scrollbar')
        $('.popup[data-popup="' + data_popup + '"]').removeClass('hide')
    });
}

function DatePicker() {
    $(document).ready(function () {
        $('.date_picker_input').pDatepicker({
            observer: true,
            format: 'YYYY/MM/DD',
            altField: '.datepicker_id',
...
function Chartjs() {
    ...
    var str = location.href, data_home = $('.data_href').attr('data-home');

    if (str == data_home + '/admin') {

        Chartbr.defaults.global.defaultFontFamily = "sans,tahoma,arial";
        Chartbr.defaults.doughnut.cutoutPercentage = 70;
        new Chartbr(
            document.getElementById("chart_visitor"), {
                "type": "line",
                "data": {
                    "labels": Label_Visitor,
                    "datasets": [{
                        label: "تعداد آی پی",
                ...
                "options": {
                    responsive: true,
                    "title": {"display": true, "text": "بازدید تعداد ورودی IP", "fontSize": 14},
                    "tooltips": {"rtl": true},
                    interaction: {interset: false,},
                    legend: false
                }
            }
        );
        ...
        new Chartbr(
            document.getElementById("chart_invoice"), {
                "type": "doughnut",
                "data": {
                    "labels": [
                        "پیش‌فاکتور",
                ...
                "options": {
                    responsive: true,
                    "title": {"display": true, "text": "فاکتورها", "fontSize": 14},
                    "tooltips": {"rtl": true},
                    interaction: {interset: false,},
                    legend: {
                        position: "bottom",
        ...
        new Chartbr(
            document.getElementById("chart_content"), {
                ...
                "options": {
                    responsive: true,
                    "title": {"display": true, "text": "تولید محتوا", "fontSize": 14},
                    "tooltips": {"rtl": true},
                    interaction: {interset: false,},
                    legend: {
                        position: "bottom",
        ...
        new Chartbr(
            document.getElementById("chart_topic"), {
                "type": "doughnut",
                "data": {
                    "labels": [
                        "تاپیک نقد و بررسی",
                ...
                "options": {
                    responsive: true,
                    "title": {"display": true, "text": "انجمن تجهیزات سخت افزاری", "fontSize": 14},
                    "tooltips": {"rtl": true},
                    interaction: {interset: false,},
                    legend: {
                        position: "bottom",
...
function TinyEditor_More() {
    tinymce.init({
        selector: 'textarea.editor-more',
        toolbar: false,
        menubar: false,
        directionality: "rtl",
        contextmenu: false,
        ...
        init_instance_callback: function (editor) {
            ...
            editor.on('keyUp', function (e) {
                /// For show/hide label when type on editor
                var id = $(tinymce.activeEditor.selection.getNode()).closest('body').data('id');
                var text = tinymce.activeEditor.getContent().length;
                if (text != 0) {
                    $('textarea#' + id).closest('.form-textarea').find("label.txt").addClass('active');
                } else {
                    $('textarea#' + id).closest('.form-textarea').find("label.txt").removeClass('active');
                }
            });
        },
        ...
        setup: function (editor) {
            editor.on("init", () => {
                $('.tox-editor-header').remove()
            });

        },

        plugins: [
            'link image imagetools table spellchecker lists directionality autoresize'
        ],
        autoresize_bottom_margin: 10,
        content_css: "/assets/css/tiny.css",
        content_style: "body { font-family:font,Helvetica,Arial,sans-serif; font-size:18px }",
...
function TinyEditor() {
    ...
    var url_rw = location.href;


    if (url_rw.indexOf('create') != -1 || url_rw.indexOf('edit') != -1 || url_rw.indexOf('admin/category/ggg') != -1) {
        ...
        tinymce.init({
            ...
            autoresize_bottom_margin: 10,
            content_css: "/assets/css/tiny.css",
            content_style: "body { font-family:font,Helvetica,Arial,sans-serif; font-size:18px; padding:0;}.mce-content-body {border-bottom:0px solid #a6a5a5}",
        });
        ...
        tinymce.init({
            ...
            plugins: [
                'link image imagetools table spellchecker lists directionality code textcolor fullscreen'
            ],
            ...
            setup: function (editor) {
                ...
                editor.ui.registry.addContextToolbar('textselection', {
                    ...
                    items: 'removeformat  lineheight formatselect bold underline indent numlist bullist image table rtl ltr link alignright aligncenter alignleft alignjustify',
                    position: 'selection',
                    scope: 'node'
                });
            ...
            block_formats: 'پاراگراف متن=p; عنوان هدینگ ۲=h2; عنوان هدینگ ۳=h3; عنوان هدینگ ۴=h4; عنوان هدینگ ۵=h5; عنوان هدینگ ۶=h6',
            ...
            autoresize_bottom_margin: 10,
            content_css: "/assets/css/tiny.css",
            content_style: "body { font-family:font,Helvetica,Arial,sans-serif; font-size:18px }.mce-content-body{padding:30px 70px !important}.mce-content-body p{line-height:3.0rem;text-align:right;font-size:1.25rem;padding:0}h1,h2,h3,h4,h5,h6,strong{margin-top:1.75rem;margin-bottom:0.75rem;font-family:sbold,tahoma;line-height:2rem;font-weight:700;}h2{font-size:1.5rem;line-height:2rem}h3{font-size:1.25rem;line-height:1.75rem;}h4{font-size:1rem;line-height:1.5rem;}h5{font-size:0.875rem;line-height:1.25rem;}h6{font-size:0.75rem;line-height:1rem;}p,blockquote{margin:0;padding:0}ul{margin:0}ul li{margin-left:2.5rem;margin-right:2.5rem;line-height:3.0rem;font-size:1.25rem;}",
        });

        tinymce.init({
            selector: 'textarea.editor-gallary',
            toolbar: 'image',
            menubar: false,
            contextmenu: false,
            elementpath: false,
            ...
            relative_urls: false,
            convert_urls: false,
            onpageload: function myCustomOnInit() {
            },
            ...
            plugins: [
                'image imagetools autoresize '
            ],

            autoresize_bottom_margin: 10,
            contextmenu: 'image',
            content_css: "/assets/css/tiny.css",
            content_style: "body { font-family:font,Helvetica,Arial,sans-serif; padding: 0 !important; margin: 0 5px !important; line-height: 35px; font-size: 1.2rem; text-align: justify; color: RGB(45, 58, 74); }",
        });
...
function Admin_Script() {
    ...
    $(".form-group input[type=radio]").click(function () {
        $(this).closest('.form-radio').find("input.type").val(this.value);
        $('.radio-show').remove();
    });

    $(document).on('click', '.form-group input[type=radio]', function () {
        $(this).closest('.form-radio').find("input.type").val(this.value);
        $('input.type[name="tax"]').val('');
        $('ul li[data-txt="arzesh-afzodeh-show"]').removeClass('active');
        $('.ul.items').removeClass('hide-form');
        $('.form-db').removeClass('input-rasmi')
        $('.form-type-wetr').addClass('form-no-enter')
    });

    $('.change_select').change(function () {
        var color = ($(this).val());
        $(this).closest('.select-option').find("input.type").val(this.value);
    })

    $(document).on('click', '.form-group-item input[type=radio]', function () {
        $(this).closest('.form-radio').find("input.type").val(this.value);
    });

    $(document).on('click', 'input[type=radio][value="Commerical"]', function () {
        $('ul li[data-txt="arzesh-afzodeh-show"]').addClass('active');
        $('.ul.items').addClass('hide-form');
        $('input.type[name="tax"]').val('arzesh-afzodeh-show');
        $('.form-db').addClass('input-rasmi');
    });

    $(document).on('click', 'input[type=radio][value="BeforeCommerical"]', function () {
        $('ul li[data-txt="arzesh-afzodeh-show"]').addClass('active');
        $('.ul.items').addClass('hide-form');
        $('input.type[name="tax"]').val('arzesh-afzodeh-show');
        $('.form-db').addClass('input-rasmi');
    ...
    $('input.data-type-bt[type="radio"]').on('click change', function (e) {
        $(this).closest('.form-radio').find("input.lastnum").val($(this).attr('data-lastnum'));
        $(this).closest('.form-radio').find("input.company").val($(this).attr('data-parent'));
        $('.lastnumber .lastnum-txt').html($('input.lastnum').val())
        $('.form-type-wetr').removeClass('form-no-enter')
    });

    $('input.data-grouplist[type="radio"]').on('click change', function (e) {
        var data_group = $(this).attr('data-group');
        $('.data-group').hide();
        $('.data-group-txt').show();
        $('.data-group[data-group="' + data_group + '"]').show();
        $('.data-group input[type="radio"]').prop('checked', false);
        $('.form-radio .type-group').val('');
        $('input.lastnum.order_no').val('');
        $('input.company').val('');
        $('.lastnumber .lastnum-txt').html('٪نامشخص٪')
        $('.chek').removeClass('hidden').addClass('show')
    });

    $('.form-checkbox .ul-lists li').click(function () {
        $(this).toggleClass('active').siblings().removeClass('active');
        $(this).parent().next('.type').val($(this).hasClass('active') ? $(this).attr('data-txt') : '')
    });

    $('ul.radio-show').click(function () {
        $('li.radio-show .message').removeClass('hidden')
    });

    $('.upload-select').change(function () {
        var filename = $('.upload-select').val();
        if (filename.substring(3, 11) == 'fakepath') {
            filename = filename.substring(12);
        }

        var file = $(this).get(0).files[0];
        var fileSize = " حجم فایل: " + Math.round(file.size / 1024) + ' KB'

        $("label[for='file_name'] b").html(filename);
        $("label[for='file_size'] b").html(fileSize);
        $("label[for='file_default']").text('نام فایل انتخاب شده:  ');
        $('.upload-start').show();
        if (filename == "") {
            $("label[for='file_default']").text('فایل انتخاب نشده است.');
            $('.upload-start').hide();
        }
    });
        ...
        $(document).ready(function () {
            ...
            function multInputs() {
                ...
                $(".txtMult").each(function () {
                    var $val1 = $('.val1', this).val().replace(/,/g, '');
                    var $val2 = $('.val2', this).val().replace(/,/g, '');
                    var $total = ($val1 * 1) * ($val2 * 1)
                    $('.multTotal', this).text(Number_Comma_Text($total));
                    val1_mult += ($val1 * 1);
                    mult += $total;
                });
                arzesh = (mult * 0.09);
                $(".grand_total").text(Number_Comma_Text(mult));
                $(".gr_arzesh_afzodeh").text(Number_Comma_Text(arzesh));
                $(".grand_total_input").val(mult);
                $(".grand_total_val").text(Number_Comma_Text(val1_mult));
                $(".grand_total_val_input").val(val1_mult);
            }
        });
    ...
    function invoice_code($type, $total_number_item) {
        var html_code = '';

        var total_number_item = $total_number_item;

        if ($type == 'invoice') {

            html_code += '<tr class="li item txtMult li-item' + total_number_item + '">';

            html_code += '<td>' + (parseInt(total_number_item) + 1) + '</td>';

            html_code += '<td class="form-textarea">';
            html_code += '<textarea name="item[' + total_number_item + '][product_name]" class="textarea editor-more type-text text-center hidden" rows="1" cols="1" aria-hidden="true" autocomplete="off"></textarea>';
            html_code += '</td>';

            html_code += '<td class="form-input sample">';
            html_code += '<input name="item[' + total_number_item + '][part_no]" class="type-text text-center" autocomplete="off" value="">';
            html_code += '</td>';

            html_code += '<td class="form-input">';
            html_code += '<input name="item[' + total_number_item + '][quantity]" class="quantity val1 number-comma type-text text-center" autocomplete="off" value="">';
            html_code += '</td>';

            html_code += '<td class="form-input ">';
            html_code += '<input name="item[' + total_number_item + '][price]" class="price val2 number-comma type-text text-center" autocomplete="off" value="">';
            html_code += '</td>';

            html_code += '<td class="form-radio form-group-item last sample">';
            html_code += '<select name="unit{{$i_count}}" autocomplete="off">';
            html_code += '<option id="unit' + total_number_item + '_2">لینک</option>';
            html_code += '<option id="unit' + total_number_item + '_1">دستگاه</option>';
            html_code += '<option id="unit' + total_number_item + '_3">بسته</option>';
            html_code += '<option id="unit' + total_number_item + '_4">عدد</option>';
            html_code += '<option id="unit' + total_number_item + '_5">قطعه</option>';
            html_code += '<option id="unit' + total_number_item + '_6">متر</option>';
            html_code += '</select>';
            html_code += '<input name="item[' + total_number_item + '][unit]" class="type hidden" type="text" value="دستگاه" autocomplete="off">';
            html_code += '</td>';

            html_code += '<td class="amount-item multTotal"></td>';

            html_code += '<td class="tools">';
            html_code += '<div class="btn btn-danger btn-delete-row" data-id="' + total_number_item + '" data-txt="شماره آیتم ' + (parseInt(total_number_item) + 1) + '"><i class="icn white float-right" style="background-image: url(assets/icons/package-x.svg);"></i>حذف آیتم ' + (parseInt(total_number_item) + 1) + '</div>';
            html_code += '<div class="dfere hidden"><input name="item[' + total_number_item + '][photo]" autocomplete="off"></div>';
            html_code += '</td>';
        ...
        if ($type == 'bill') {
            ...
            html_code += '</td>';
            html_code += '<td class="form-input sample">';
            html_code += '<input name="item[' + total_number_item + '][part_no]" class="type-text val2 text-center" autocomplete="off" value="">';
            html_code += '</td>';
            html_code += '<td class="form-input">';
            html_code += '<input name="item[' + total_number_item + '][unit]" class="unit val1 number-comma0 type-text text-center" autocomplete="off" value="">';
            html_code += '</td>';
            html_code += '<td class="tools">';
            html_code += '<div class="btn btn-danger btn-delete-row" data-id="' + total_number_item + '" data-txt="شماره آیتم ' + (parseInt(total_number_item) + 1) + '"><i class="icn white float-right" style="background-image: url(assets/icons/package-x.svg);"></i>حذف آیتم ' + (parseInt(total_number_item) + 1) + '</div>';
            html_code += '<div class="dfere hidden"><input name="item[' + total_number_item + '][photo]" autocomplete="off"></div>';
    ...
    $('.more-list-btn .btn').click(function () {
        var total_number_item = $('.total_number_item').val();
        var data_type = $('.more-list-btn').attr('data-type')
        $('.more-list-btn').addClass('hide')
        $('.total_number_item').val(parseInt(total_number_item) + 1);
        $('.total-count-txt').html(parseInt(total_number_item) + 1);
        $('.total_item_input').val(parseInt(total_number_item) + 1);
        $('.btn-delete-row').removeClass('hidden')
        $('.table-o table tbody').append(invoice_code(data_type, total_number_item));
        $(".type-text").keyup(function () {
            var input_bl = $(this).val();
            if (input_bl == 0) {
                $(this).next().removeClass('active');
    ...
    $(document).ready(function () {
        let url_admin = location.href;
        if (url_admin.indexOf('/admin/') !== -1) {
            var part_admin = url_admin.replace(data_home + '/admin/', '');
            console.warn('Couldn\'t find ' + '.nav-admin ul li.' + part_admin + ' a');
            // $('.nav-admin ul li.' + part_admin + ' a').addClass('active');
        }
    });

    $(document).on('click', '.notifaction .bell-pop', function () {
        $('.bell-btn').removeClass('active');
        $('.bell-pop').removeClass('active');
    });
    $(document).on('click', '.notifaction .bell-btn', function () {
        $('.bell-btn').addClass('active');
        $('.bell-pop').addClass('active');
    });
    $(document).on('click', '.notifaction .bell-btn.active', function () {
        $('.bell-btn').removeClass('active');
        $('.bell-pop').removeClass('active');
    });
    ...
    $(".chkbx-catlist").change(function () {
        var text = "";
        var data_txt = "";
        $(".chkbx-catlist:checked").each(function () {
            text += $(this).val() + ',';
            data_txt += $(this).attr('data-txt') + " ، ";
        });
        text = text.substring(0, text.length - 1);
        data_txt = data_txt.substring(0, data_txt.length - 2);
        $(".btn-ok-category").attr('data-txt', data_txt).attr('data-id', text);
    });

    $(document).ready(function () {
        $(".chkbx-list").trigger("change");
        $(".chkbx-list").change(function () {
            var text = "";
            var data_txt = "";
            var data_html = "";
            var data_popup = $(this).attr('data-popup');
            var data_ext = $(this).attr('data-ext');
            $('.chkbx-list[data-popup="' + data_popup + '"]:checked').each(function () {
                text += $(this).val() + ',';
                data_txt += $(this).attr('data-txt') + ',';
                data_html += $(this).attr('data-txt') + "." + data_ext + " ، ";
            });
            text = text.substring(0, text.length - 1);
            data_txt = data_txt.substring(0, data_txt.length - 1);
            data_html = data_html.substring(0, data_html.length - 2);
            var data_txt = data_txt.split(',').filter(function (item, pos, self) {
                return self.indexOf(item) == pos;
            });
            var data_html = data_html.split(',').filter(function (item, pos, self) {
                return self.indexOf(item) == pos;
            });
            var text = text.split(',').filter(function (item, pos, self) {
                return self.indexOf(item) == pos;
            });
            $('.chkbx-selected-list[data-popup="' + data_popup + '"]').val(data_txt)
        });
    ...
    $('.typer-get').keyup(function () {
        var value_sku_number_get = $(this).val();
        var value_sku_get = '';
        var val = value_sku_get + value_sku_number_get
        $('.btn-ok-part-no').attr('data-sku-number-get', val).attr('data-sku-get', value_sku_get)
        $('.out-box span').html(val);

        $('.out-box span').html(encodeURI(val.replace(/((\s*\S+)*)\s*/, "$1").replace(/\s+/g, "-"))).css({'color': 'rgb(84, 84, 84)'});

    }).keyup();
    $('select.selectoption').change(function () {
        var value_sku_number_get = $('.typer-get').val();
        var value_sku_get = $(this).val();
        var val = value_sku_get + value_sku_number_get;
        $('.btn-ok-part-no').attr('data-sku-number-get', val).attr('data-sku-get', value_sku_get)
        $('.out-box span').html(val);
    });

    $('input.type-input-jqu').on('input', function () {
        var value = $(this).val();
        if ((value !== '') && (value.indexOf('.') === -1)) {
        }
    });

    $(document).on('click', '.btn-ok-part-no', function () {
        var data_sku = $(this).attr('data-sku-number-get')
        var data_id = $(this).attr('data-sku-get')
        $('h2.sku-number-txt').html(data_sku);
        $('.sku_n').val(data_sku);
        $('.sku_id').val(data_sku);
    });

    $(document).on('click', '.btn-ok-category', function () {
        var data_txt = $(this).attr('data-txt')
        var data_id = $(this).attr('data-id')
        $(".catlist-txt").html(data_txt);
        $(".catlist-input").val(data_id);
    });

    choosePrimaryImageFunction = function (input) {
        var selectedImagesWithoutClicked = $('.chkbx-selected-list[data-popup="gallary"]').val().split(',').filter((word) => word != input);
        var selectedImagesWithoutClickedString = input + ',' + selectedImagesWithoutClicked.join(',');
        // console.info('input',input);
        // console.info('selectedImagesWithoutClicked',selectedImagesWithoutClicked);
        // console.info('selectedImagesWithoutClickedString',selectedImagesWithoutClickedString);
        $('.chkbx-selected-list[data-popup="gallary"]').val(selectedImagesWithoutClickedString);
        $('[name=thumb]').val(selectedImagesWithoutClickedString);
        // $('.btn-ok').trigger('click');
    };
    $(document).on('click', '.btn-ok', function () {
        var data_popup = $(this).attr('data-popup')
        var data_txt = $('.chkbx-selected-list[data-popup="' + data_popup + '"]').val()
        var data_html = $(this).attr('data-html')
        var data_id = $(this).attr('data-id')
        var data_type = $('.popup .body').attr('data-type')

        var chkbx_selected_list = $(this).next().val()

        if (chkbx_selected_list) {
            $('.list-txt[data-popup="' + data_popup + '"]').html(data_html);
            var list_input = data_txt.split(',').filter((word, i, arr) => arr.indexOf(word) === i).map(function (word, i, arr) {
                return word;
            });
            $('input.select-list').val(list_input);
            if (data_popup != 'gallary') {
                var list_txt = data_txt.split(',').filter((word, i, arr) => arr.indexOf(word) === i).map(function (word, i, arr) {
                    // return '<div class="txt"><span>' + word + '.'+ data_popup +'</span><input type="checkbox" class="delete_item" title="حذف" name="delete_item_chkbox" data-popup="'+ data_popup +'" value="' + word + '" checked="checked" data-type="' + data_type + '"></div>';
                    return '<div class="txt"><span>' + word + '.' + data_popup + '</span></div>';
                });
            } else {
                var list_txt = data_txt.split(',').filter((word, i, arr) => arr.indexOf(word) === i).map(function (word, i, arr) {
                    return '<li class="thumb-btn float-right rwe"><a onclick="choosePrimaryImageFunction(\''+ word +'\')" href="javascript:void(0)"><img class="thumb-s lazyload float-right-0" src="/generated/images/' + data_type + '/80x80/' + word + '.webp" alt="' + word + '" width="80" height="80"></a></li>';
                });
                // var list_txt = data_txt.split(',').filter((word, i, arr) => arr.indexOf(word) === i).map(function(word, i, arr) { return '<li class="thumb-btn float-right rwe"><img class="thumb-s lazyload float-right-0" src="/images/uploads/picture/' + data_type + '/' + word + '.jpg" alt="' + word + '" width="80" height="80"><input type="checkbox" class="delete_item" title="حذف" name="delete_item_chkbox" data-popup="'+ data_popup +'" value="' + word + '" checked="checked" data-type="' + data_type + '"></li>'; });
                var list_thumb = data_txt.split(',').filter((word, i, arr) => arr.indexOf(word) === 0).map(function (word, i, arr) {
                    return '<img class="thumb float-right wewt" src="/generated/images/' + data_type + '/340x170/' + word + '.webp" alt="' + word + '" width="340" height="170">';
                });
                $('.figure .images').html(list_thumb.join(''))
            }
            // console.info('list_txt',list_txt);
            // console.info('list_thumb',list_thumb);
            $('.list-txt[data-popup="' + data_popup + '"]').html(list_txt.join(''));
            $('.list-input[data-popup="' + data_popup + '"]').val(data_txt);
            try {
                $('[name=thumb]').val(data_txt);
                console.info('thumb updated');
                console.info(data_txt);
            } catch (ignoredError) {
                console.warn(ignoredError);
            }
        }
    });
    ...
    $('input.input').keyup(function () {
        var data_txt = $(this).attr('data-txt')
        var val = $('input.input[data-txt="' + data_txt + '"]').val()
        if (val == '') {
            $('.popup .form-group[data-txt="' + data_txt + '"] label').fadeOut()
        } else {
            $('.popup .form-group[data-txt="' + data_txt + '"] label').fadeIn()
        }
    }).keyup();

    function checkURL(value, data_id) {
        const CSRF_TOKEN = $('.data_href').attr('csrf-token');
        if (value.length > 1) {
            $.ajax({
                cache: false,
                url: '/admin/check-url/fetch/' + encodeURI(value.replace(/((\s*\S+)*)\s*/, "$1").replace(/\s+/g, "-")) + '?CSRF_TOKEN=' + CSRF_TOKEN + '&_=' + Math.random(),
                type: 'POST',
                data: {"_token": CSRF_TOKEN, data_id: data_id},
                success: function (data) {
                    if (data == 1) {
                        $('.result-txt-check').html('')
                        $('.check-url-result .icn.ico-ok').removeClass('show')
                        $('.check-url-result .icn.ico-x').removeClass('show')
                        $('.check-url-result .icn.ico-x').addClass('show')
                        $('.result-txt-check').html('<span class="badge badge-info red low">خطا: آدرس تکراری است</span>')
                        $('button.btn[form="Form_Save"]').attr('disabled', true).addClass('disabled');
                    } else {
                        $('.result-txt-check').html('')
                        $('.check-url-result .icn.ico-x').removeClass('show')
                        $('.check-url-result .icn.ico-ok').removeClass('show')
                        $('.check-url-result .icn.ico-ok').addClass('show')
                        $('button.btn[form="Form_Save"]').attr('disabled', false).removeClass('disabled');
                    }
                }
            });
        }
    ...
    $('.title-get').keyup(function () {
        var value = $(this).val();
        var data_id = $(this).attr('data-id');
        checkURL(value, data_id)
    });

    $('.title-get').keyup(function () {
        var value = $(this).val();
        var data_id = $(this).attr('data-id');
        $('.link-slug-get.create').val(value.replace(/((\s*\S+)*)\s*/, "$1").replace(/\s+/g, "-")).css({'color': 'rgb(84, 84, 84)'});
        $('.slug-get.create').val(value.replace(/((\s*\S+)*)\s*/, "$1").replace(/\s+/g, "-"));
        $('.link-slug-get.create').val(value.replace(/((\s*\S+)*)\s*/, "$1").replace(/\s+/g, "-"));
        $('.link-input-get.create').html(value.replace(/((\s*\S+)*)\s*/, "$1").replace(/\s+/g, "-"));
    });


    $('.title-get-kit').keyup(function () {
        var value = $(this).val();
        var data_id = $(this).attr('data-id');
        $('.btn-ok-title').attr('data-title', value)
        checkURL(value, data_id)

    });

    $(document).on('click', '.btn-ok-title', function () {
        var data_title = $(this).attr('data-title')
        $('h1.title-s').html(data_title)
        $('input.title-s').val(data_title)
    });

    $('.slug-get').keyup(function () {
        var value = $(this).val()
        var data_id = $(this).attr('data-id');
        $('.slug-get').removeAttr("style");
        checkURL(value, data_id)
    }).keyup();

    $(document).on('click', '.btn-ok-title', function () {
        var data_slug = $(this).attr('data-slug')
        $('h1.slug-s').html(data_slug)
        $('input.slug-s').val(data_slug)
        $('input.link-slug-get').val(data_slug)
        $('.link-input-get').html($('.slug-get').val().replace(/((\s*\S+)*)\s*/, "$1").replace(/\s+/g, "-"))
        $('.link-slug-get').val($('.slug-get').val().replace(/((\s*\S+)*)\s*/, "$1").replace(/\s+/g, "-"))

    });

    // $(document).on('change', 'input.delete_item[name=delete_item_chkbox]', function(){
    //   var data_popup = $(this).attr('data-popup')
    //   var data_type = $(this).attr('data-type')
    //   $('.list-input[data-popup="'+ data_popup +'"]').val( $('input.delete_item[data-popup="'+ data_popup +'"]:checked').map(function() { return $(this).val(); }).get().join(',') );
    //   $(this).parent().remove();
    //   var data_txt =  $('.list-input[data-popup="'+ data_popup +'"]').val( );
    //   if ( data_popup == 'gallary' ) {
    //     if ( data_txt ) {
    //       var list_thumb = data_txt.split(',').filter((word, i, arr) => arr.indexOf(word) === 0).map(function(word, i, arr) { return '<img class="thumb float-right wety" src="/images/uploads/picture/' + data_type + '/' + word + '.jpg" alt="' + word + '" width="400">'; });
    //       $('.figure .images').html( list_thumb.join('') )
    //     } else {
    //
    //       if ( data_type == 'product' ){
    //         $('.figure .images').html( '<img class="thumb float-right qwr" src="/assets/lazy/gallary.jpg" width="400">' )
    //       } else {
    //         $('.figure .images').html( '<img class="thumb float-right wrtyq" src="/assets/lazy/post.jpg" width="400">' )
    //       }
    //
    //     }
    //   }
    //   if ( data_popup == 'pdf' ) {
    //     if (data_txt == '' ) {
    //       $('.attach .list-txt[data-popup="'+ data_popup +'"]').html('فایل ضمیمه PDF اضافه نشده.')
    //     }
    //   }
    //   if ( data_popup == 'zip' ) {
    //     if ( data_txt == '' ) {
    //       $('.attach .list-txt[data-popup="'+ data_popup +'"]').html('فایل ضمیمه ZIP اضافه نشده.')
    //     }
    //   }
    //
    // });

    $(document).on('click', '.btn-password-generate', function () {
        ...
        $('.input-password-generate').val(password)
        $('.btn-copy-generate').removeClass('hidden')
    });

    $(document).on('click', '.btn-copy-generate', function () {
        var copyText = $('.input-password-generate');
        copyText.select();
        document.execCommand("copy");
    });

    $(document).on('click', '.wie', function () {
        if ($(this).attr('data-option') == 'hide') {
            $(this).removeAttr('data-option')
            $('.box-wie').removeClass('hidden');
            $('.input-wie').val('')
            $('.span-wie').html('')
            $('.select-wie').prop("selectedIndex", 0);
        } else {
            $(this).attr('data-option', 'hide')
            $('.input-wie').val('-')
            $('.span-wie').html('-')
            $('.box-wie').addClass('hidden');

            $('.btn-close-popup').attr('data-sku-get', '-').attr('data-sku-number-get', '-')
        }
...
$(document).on('click', '.btn-delete-row', function () {
    ...
    $val1 = $('.li.li-item' + data_id + ' .val1');
    $val2 = $('.li.li-item' + data_id + ' .val2');
    $multTotal = $('.li.li-item' + data_id + ' .multTotal');

    var val1 = $val1.val().replace(/[^\d.]/g, '');
    var val2 = $val2.val().replace(/[^\d.]/g, '');

    var total_item_input = $('input.total_item_input').val().replace(/[^\d.]/g, '');  //parseInt
    var total_val_input = $('input.grand_total_input').val().replace(/[^\d.]/g, '');  //parseInt
    var grand_total_val_input = $('input.grand_total_val_input').val().replace(/[^\d.]/g, '');  //parseInt

    AlertConfirm('حذف لیست', 'آیا میخواهید ' + data_txt + ' را حذف شود؟', 'بله!')

    $(document).ready(function () {
        $('.btn-yes-done').on('click', function () {
            setTimeout(function () {

                $('.total-count-txt').text(total_item_input - 1)
                $('.total_item_input').val(total_item_input - 1)
                if (total_item_input == '1') {
                    $('.btn-delete-row').addClass('hidden')
                }

                var res_total = parseFloat(total_val_input) - (val1 * val2);
                $(".grand_total").text(Number_Comma_Text(res_total));
                $(".grand_total_input").val(res_total);

                var res_pro = parseFloat(grand_total_val_input) - (val1 * 1);
                $(".grand_total_val").text(Number_Comma_Text(res_pro));
                $(".grand_total_val_input").val(res_pro);

                $val1.val('0');
                $val2.val('0');
                $multTotal.text('0');
                $('.li.li-item' + data_id + ' textarea').html('--REMOVE--');
                $('.li.li-item' + data_id + ' input').val('--REMOVE--');
                $('.li.li-item' + data_id + ' input.number-comma').val('0');
                $('.li.li-item' + data_id + ' .remove-tyew').val('--REMOVE--');

                $('.li.li-item' + data_id).hide();
...
$(document).on('click', '.delete-btn-sql', function (event) {
    event.preventDefault();
    var data_id = $(this).attr('data-id');
    var data_txt = $(this).attr('data-txt');

    AlertConfirm('حذف', 'آیا میخواهید ' + data_txt + ' را حذف شود؟', 'بله!')

    $(document).ready(function () {
        $('.btn-yes-done').on('click', function () {
            $('.delete-form-' + data_id).submit();
        });
    });

});

$(document).on('click', '.delete-btn-disable', function (event) {
    event.preventDefault();
    alert('متاسفم، این ابزار برای شما غیرفعال می‌باشد.')
});
$(document).on('click', '.add-page', function () {
    var total_number_item = $('.total_number_item').val();
    $('.total_number_item').val(parseInt(total_number_item) + 1);
    var html_code = '';
    html_code += '<section class="card lis item head form-editor float-right w-full" style="margin: 0 0 20px 0;">';
    html_code += '<div class="panel float-right w-full">';
    html_code += '<div class="tab-content direction-rtl float-right">';
    html_code += '<div class="pan-tabs">';
    html_code += '<ul class="tabs-list float-right">';
    html_code += '<li class="tab descr bold first active" data-id="1">';
    html_code += '<i class="icn fill float-right" style="background-image: url(/assets/icons/glasses.svg);"></i>';
    html_code += 'توضیحات دیتاشیت (صفحه <span class="no-page">' + (parseInt(total_number_item) + 2) + '</span>)';
    html_code += '</li>';
    html_code += '</ul>';
    html_code += '</div>';
    html_code += '<div class="data tab-content desc  float-right direction-rtl">';
    html_code += '<div class="content-i data-1 show">';
    html_code += '<div class="form-textarea float-right w-full">';
    html_code += '<textarea name="item[' + total_number_item + '][description]" class="textarea editor-content type-text tetwe" autocomplete="off" placeholder="  توضیحات دیتاشیت را تایپ کنید ..." title="توضیحات"></textarea>';
    html_code += '</div>';
    html_code += '</div>';
    html_code += '</div>';
    html_code += '</div>';
...
$(document).on('click', '.delete-page', function (event) {
    ...
    AlertConfirm('حذف صفحه', 'آیا میخواهید صفحه ' + data_id + ' را حذف شود؟', 'بله!')
    $(document).ready(function () {
        $('.btn-yes-done').on('click', function () {
            setTimeout(function () {
                $('section.item[data-id=' + data_id + ']').remove()
                $('.total_number_item').val(parseInt(total_number_item) - 1);
            }, 100)
        });
    });
});
$('.popup .body').on('scroll', function () {
    var div = $(this).get(0);
    data_popup = $(this).attr('data-popup'),
        data_fetch = $(this).attr('data-fetch'),

        elm = $('.popup[data-popup=' + data_popup + '] .body'),
        data_start = elm.attr('data-str'),
        data_limit = elm.attr('data-limit'),
        data_id = elm.attr('data-id'),
        data_type = elm.attr('data-type'),
        data_slug = elm.attr('data-slug');

    if (data_fetch == 'scroll') {
        if (div.scrollTop + div.clientHeight >= div.scrollHeight) {

            $('.popup .loading').remove();
            elm.append("<div class='loading'>Loading...</div>");

            data_start = parseInt(data_start) + parseInt(data_limit);
            elm.attr('data-str', data_start);

            $.post('admin/fetch/ajax/' + data_popup + '?CSRF_TOKEN=' + CSRF_TOKEN + '&_=' + Math.random(),
                {
                    _token: CSRF_TOKEN,
                    ...
                    if (data != '') {
                        $('.popup[data-popup=' + data_popup + '] ul').append(data)
                        $('input.chkbx-list[data-popup="' + data_popup + '"]').change(function () {
                            $('.chkbx-selected-list[data-popup="' + data_popup + '"]').val(
                                $('.chkbx-list[data-popup="' + data_popup + '"]:checked').map(function () {

                                    return $(this).attr('data-txt');
                                }).get().join(',')
                            );
                        });
                        $('.popup[data-popup="' + data_popup + '"] .lazyload-open').removeClass('lazyload-open').addClass('lazyload')
                    } else {
                        $('.popup .loading').remove();
                        $('.popup[data-popup=' + data_popup + '] .body').removeAttr('data-fetch').removeAttr('data-popup')
                    }
