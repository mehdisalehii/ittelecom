                ...
                <div class="arch-content">
                    <a class="block" href="/blog/{{$row->slug}}" title="{{$row->title}}">
                        ...
                        @foreach( explode(',',$row->thumb) as $thumb => $val)
                            ...
                            @endif
                        @endforeach
                        <div class="para">{!! mb_substr( strip_tags( str_replace('   ','',$row->content)) ,0,250, "utf-8") !!}...<span class="more bold" title="ادامه مطلب {{$row->title}}">ادامه مطلب</span></div>
                    </a>
                </div>
            </article>
        </li>
