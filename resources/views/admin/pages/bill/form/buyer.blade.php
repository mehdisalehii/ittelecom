...
<div class="label-txt">نام فروشنده:</div>
    ...
    @foreach($companys as $row_com)
        @if($row_com->parentId == $bill->company_id)
    ...
    <input class="type hidden" type="text" name="company_id" value="{{$bill->company_id}}" autocomplete="off">
    <input class="lastnum hidden" type="text" name="order_no" value="{{$bill->order_no}}" autocomplete="off">
</div>
<div class="form-radio form-group last chek">
<div class="label-txt data-group-txt">نوع حواله‌:</div>

    @php $i = 0; @endphp
    @foreach($companys as $row_com)
        @if($bill->bill_type == $row_com->type)
    ...
    <input class="type hidden" type="text" name="bill_type" value="{{$bill->bill_type}}" autocomplete="off">
</div>
...
@endforeach
    ...
    <input class="type type-group hidden" type="text" name="bill_type" autocomplete="off">
</div>
